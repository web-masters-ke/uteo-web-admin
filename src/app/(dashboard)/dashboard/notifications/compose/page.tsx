...
export default function ComposeNotificationPage() {
  ...
  const fetchTemplates = useCallback(async () => {
    try {
      const res = await import('@/lib/api').then(m => m.default.get('/admin/notification-templates'));
  ...
  const saveAsTemplate = async () => {
    if (!tmplName.trim() || !title.trim()) return;
    try {
      await import('@/lib/api').then(m => m.default.post('/admin/notification-templates', {
        name: tmplName, type, subject: title, body: message,
      }));
      addToast('success', 'Template saved');
      setTmplName(''); setShowSaveTmpl(false);
      fetchTemplates();
    } catch { addToast('error', 'Failed to save template'); }
  };

  const loadTemplate = (t: NotificationTemplate) => {
    setTitle(t.subject); setMessage(t.body); setType(t.type); setShowTemplates(false);
  };

  const deleteTemplate = async (id: string) => {
    try {
      await import('@/lib/api').then(m => m.default.delete(`/admin/notification-templates/${id}`));
        ...
        <div className="lg:col-span-3 bg-card rounded-xl border border-border p-6 space-y-5">
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              {templates.length > 0 && (
                <button onClick={() => setShowTemplates(!showTemplates)} className="text-xs px-3 py-1.5 rounded-lg border border-border hover:bg-muted transition-colors flex items-center gap-1">
                  <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 10h16M4 14h16M4 18h16" /></svg>
                  Templates ({templates.length})
                </button>
              )}
              <button onClick={() => setShowSaveTmpl(true)} className="text-xs px-3 py-1.5 rounded-lg border border-[#F77B0F]/50 text-[#F77B0F] hover:bg-[#F77B0F]/10 transition-colors flex items-center gap-1">
          ...
          {showTemplates && (
            <div className="rounded-lg border border-border bg-muted/30 divide-y divide-border">
              {templates.map(t => (
                <div key={t.id} className="flex items-center justify-between px-3 py-2 hover:bg-muted/50 transition-colors">
                  <button onClick={() => loadTemplate(t)} className="flex-1 text-left">
                    <p className="text-sm font-medium">{t.name}</p>
                    <p className="text-xs text-muted-foreground truncate">{t.subject}</p>
                  </button>
                  <button onClick={() => deleteTemplate(t.id)} className="text-muted-foreground hover:text-red-500 p-1">
          ...
          {showSaveTmpl && (
            <div className="rounded-lg border border-[#F77B0F]/30 bg-[#F77B0F]/5 p-3 space-y-2">
              <p className="text-xs font-semibold text-[#F77B0F]">Save as Template</p>
              <input value={tmplName} onChange={e => setTmplName(e.target.value)} placeholder="Template name..." className={ic} />
              <div className="flex gap-2">
                <button onClick={() => { setShowSaveTmpl(false); setTmplName(''); }} className="flex-1 py-1.5 text-xs rounded-lg border border-border hover:bg-muted transition-colors">Cancel</button>
                <button onClick={saveAsTemplate} className="flex-1 py-1.5 text-xs rounded-lg bg-[#F77B0F] text-white font-semibold hover:bg-[#e06a0d] transition-colors">Save</button>
              </div>
            </div>
          )}
