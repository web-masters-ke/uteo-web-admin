...
import { LineTrend, ColorBar, PieBreakdown, HorizBar, MultiBar } from '@/components/Charts';
...
function MetricCard({ label, value, sub, dot, href }: {
...
function SectionHeader({ title, sub, href }: { title: string; sub?: string; href?: string }) {
  return (
    <div className="flex items-center justify-between mb-4">
      ...
      {href && (
        <Link href={href} className="text-xs font-semibold text-[#F77B0F] hover:underline flex items-center gap-1">
          View all
...
const QUICK_LINKS = [
  ...
  { label: 'Broadcasts', href: '/dashboard/notifications', icon: 'M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9' },
...
const FUNNEL_ORDER = ['PENDING', 'UNDER_REVIEW', 'SHORTLISTED', 'INTERVIEW', 'OFFERED', 'HIRED', 'REJECTED'];
  ...
  const fetchData = async (silent = false) => {
    ...
    try {
      const [dashData, jobsData, appsData, usersData, analyticsData] = await Promise.all([
        ...
        jobService.list({ limit: 50, page: 1 }).catch(() => ({ items: [], total: 0 })),
      ...
      setRecentJobs((jobsResult.items ?? []).slice(0, 10));
      setRecentApplications((appsResult.items ?? []).slice(0, 10));
      setRecentUsers(usersResult.items ?? []);

      // Sign-ups area chart
      if (analyticsData?.signupsPerDay?.length) {
        setSignupChart(analyticsData.signupsPerDay.map((d: { date: string; count: number }) => ({ date: d.date.slice(5), value: d.count })));
      }
      ...
      setAppStatusPie(Object.entries(statusCounts).map(([name, value]) => ({ name: name.replace(/_/g, ' '), value })));

      // Jobs by type — colored bar
      const typeMap: Record<string, number> = {};
      (jobsResult.items ?? []).forEach((j: AdminJob) => {
        const t = (j.jobType || 'OTHER').replace(/_/g, ' ');
        typeMap[t] = (typeMap[t] || 0) + 1;
      });
      setJobTypeBar(Object.entries(typeMap).map(([name, value]) => ({ name, value })));

      // Hiring funnel — horizontal bar
      const funnel = FUNNEL_ORDER
        .map((s, i) => ({ name: s.replace(/_/g, ' '), value: statusCounts[s] || 0, _i: i }))
        .filter(d => d.value > 0);
      setFunnelBar(funnel);
    ...
    <div className="space-y-7 pb-10">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {loading ? (
          ...
          <>
            <MetricCard label="Job Seekers" value={formatNumber(stats.totalUsers)} sub="Registered candidates" dot={BRAND} href="/dashboard/users" />
            <MetricCard label="Active Jobs" value={formatNumber(stats.totalJobs)} sub="Open positions" dot="#10b981" href="/dashboard/jobs" />
            <MetricCard label="Applications" value={formatNumber(stats.totalApplications)} sub="Total submissions" dot="#8b5cf6" href="/dashboard/applications" />
            <MetricCard label="Companies" value={formatNumber(stats.totalCompanies)} sub="Hiring employers" dot="#06b6d4" href="/dashboard/companies" />
          </>
        ) : null}
      </div>

      {/* ── Row 1: Sign-ups line + App status solid pie ────── */}
        ...
        {loading ? (
          ...
          <>
            ...
            <PieBreakdown
              ...
              subtitle="Breakdown by pipeline stage"
      ...
      {/* ── Row 2: Jobs by type bar + Hiring funnel horiz bar ─ */}
        ...
        {loading ? (
          ...
          <>
            <ColorBar
              ...
              subtitle="Distribution of job types across all postings"
              data={jobTypeBar.length ? jobTypeBar : [
            ...
            <HorizBar
              title="Hiring Funnel"
              subtitle="Applications across pipeline stages"
              data={funnelBar.length ? funnelBar : FUNNEL_ORDER.slice(0, 5).map(s => ({ name: s.replace(/_/g, ' '), value: 0 }))}
      ...
      {/* ── Row 3: Recent Jobs + Recent Applications tables ─── */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">

        {/* Recent Job Postings */}
        <div className="bg-card border border-border rounded-xl overflow-hidden">
          <div className="px-5 py-4 border-b border-border">
            <SectionHeader title="Recent Jobs" sub="Latest job postings" href="/dashboard/jobs" />
          </div>
          {loading ? (
            ...
            <div className="py-12 text-center">
              <svg className="w-10 h-10 mx-auto text-muted-foreground/30 mb-2" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4" /></svg>
        ...
        <div className="bg-card border border-border rounded-xl overflow-hidden">
          <div className="px-5 py-4 border-b border-border">
            <SectionHeader title="Recent Applications" sub="Latest submissions" href="/dashboard/applications" />
          </div>
          {loading ? (
            <div className="divide-y divide-border">
              {Array.from({ length: 5 }).map((_, i) => (
                <div key={i} className="px-5 py-3 animate-pulse flex gap-3">
                  ...
                  <div className="flex-1 space-y-2"><div className="h-3 bg-muted rounded w-40" /><div className="h-2 bg-muted rounded w-28" /></div>
                </div>
              ))}
            </div>
          ) : recentApplications.length === 0 ? (
            <div className="py-12 text-center">
              <svg className="w-10 h-10 mx-auto text-muted-foreground/30 mb-2" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
        ...
        {loading ? (
          <div className="divide-y divide-border">
            {Array.from({ length: 6 }).map((_, i) => (
              <div key={i} className="px-5 py-3 animate-pulse flex gap-3">
                <div className="h-8 w-8 rounded-full bg-muted flex-shrink-0" />
                <div className="flex-1 space-y-2"><div className="h-3 bg-muted rounded w-36" /><div className="h-2 bg-muted rounded w-48" /></div>
              </div>
            ))}
          </div>
        ) : recentUsers.length === 0 ? (
          <div className="py-10 text-center"><p className="text-sm text-muted-foreground">No users yet</p></div>
        ) : (
          <div className="divide-y divide-border">
            {recentUsers.map(u => (
      ...
      <div>
        ...
        <div className="grid grid-cols-4 sm:grid-cols-8 gap-3">
          {QUICK_LINKS.map(ql => (
            <Link key={ql.href} href={ql.href} className="flex flex-col items-center gap-2 p-3 bg-card border border-border rounded-xl hover:border-[#F77B0F]/40 hover:bg-[#F77B0F]/5 transition-all group text-center">
              <div className="w-9 h-9 rounded-lg bg-[#F77B0F]/8 flex items-center justify-center group-hover:bg-[#F77B0F]/15 transition-colors">
                <svg className="w-5 h-5 text-[#F77B0F]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.75}>
